"""Nemo's online probe: how well its unit embeddings separate brain regions.

A training diagnostic and not a benchmark number. Every val epoch it fits a logistic
probe over pretrain units, holding out whole subject groups at a time, and its
``train/l1subout_br/f1_macro`` is what Nemo's early stopping reads. TS3 scores saved
embeddings under its own rules, so this is a deliberate copy rather than shared code:
the table below reads the pretrain regime and has no eval branch to reach for, which is
what keeps the scored units out of a pretraining run.
"""
...
class BrainRegionMonitor:
    """Leave-subject-group-out region probe over Nemo's pretrain-unit embeddings.
